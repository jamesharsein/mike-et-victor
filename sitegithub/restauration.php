...
if (isset($_POST['envoyer'])) {
    if (!empty($_POST['nom']) && !empty($_POST['prenom']) && !empty($_POST['mail']) && !empty($_POST['age']) && !empty($_POST['numero']) && !empty($_POST['sexe']) && !empty($_POST['profession']) && !empty($_POST['message'])) {
        ...
        $insertUser = $db->prepare('INSERT INTO restauration(nom,prenom,mail,age,numero,sexe,profession,message)VALUES(?,?,?,?,?,?,?,?)');
        $insertUser->execute(array($nom, $prenom, $mail, $age, $numero, $sexe, $profession, $message));

        $recupUser = $db->prepare('SELECT * FROM restauration WHERE nom = ? AND prenom = ? AND mail = ? AND age = ? AND numero =? AND sexe = ? AND profession = ? AND message= ? ');
...
<style>
    ...
    ul li {
        ...
        font-size: 70px;
        letter-spacing: 15px;
        animation: flash 17s linear infinite;
    }
